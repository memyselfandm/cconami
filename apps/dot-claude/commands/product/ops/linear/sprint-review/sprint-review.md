---
allowed-tools: Bash(date:*), Bash(git log:*), Bash(git diff:*), Bash(linctl:*), Todo, Task, Read, Grep, Glob
...
1. Use the Task tool to execute the `sprint_context_subagent_prompt` below, replacing `$PROJECT_NAME` with the actual project identifier
   <sprint_context_subagent_prompt>
      ...
      1. **Fetch Sprint Project Details**:
         - Use `linctl team list --json` to get team context
         - Use `linctl project list --json` to find the project with identifier $PROJECT_NAME
         - Extract project ID from the matching sprint project
         - Get project description and status

      2. **Fetch All Sprint Issues**:
         - Use `linctl issue list --project <project-id> --json` to get all issues in the sprint
         - For each issue:
           - Fetch complete issue details with `linctl issue get <issue-id> --json` including:
             - Description and acceptance criteria
             - Labels (especially phase:*, area:*, type:*)
             - Priority level and current state
             - Parent/child relationships
           - If issue has children, use `linctl issue list --parent <issue-id> --json` to fetch sub-issues
         - Build complete issue hierarchy

      3. **Extract Agent Completion Claims**:
         For each issue with comments:
         - Use `linctl comment list --issue <issue-id> --json` to fetch all comments
...
2. **Update Linear Issues** (if confirmed):
   ...
   linctl comment create issue_id --body f"""## Sprint Review Validation
...
"""
   ...
   if recommendation == "REJECT" or recommendation == "REWORK":
       ...
       # Note: Add labels using --label flag if needed
   ```

3. **Add Sprint Project Summary**:
   ```python
   # Add summary comment to sprint project using project identifier
   linctl comment create <project-identifier> --body f"""## Sprint Review Complete
